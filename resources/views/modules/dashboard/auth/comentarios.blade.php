...
<div class="max-w-7xl mx-auto px-6 py-10">
    <h1 class="text-3xl font-bold mb-8 text-white">Gestión de Comentarios</h1>
@if(session('success'))
    ...
    <table class="min-w-full text-left text-sm text-white bg-[#2a2c42]">
        ...
        <tbody>
            ...
            <tr class="border-t border-[#3d3f56]">
                ...
                <td class="px-4 py-2">
                    <form action="{{ route('admin.comentarios.eliminar', $comentario->id) }}" method="POST" onsubmit="return confirm('¿Eliminar este comentario?')">
                        @csrf
                        @method('DELETE')
                        <button class="bg-red-600 hover:bg-red-500 text-white px-3 py-1 rounded">Eliminar</button>
                    </form>
                </td>
            </tr>
            @endforeach
